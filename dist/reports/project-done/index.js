...
function process(config, issueList, drillIn) {
    ...
    const cardsForType = completedCards.cardType === '*'
        ? issues
        : rptLib.filterByLabel(issues, completedCards.cardType.toLowerCase());
    completedCards.cards = cardsForType.filter(issue => issue['project_done_at'] && moment_1.default(issue['project_done_at']).isAfter(daysAgoMoment));
...
function renderMarkdown(targets, processedData) {
    ...
    for (const card of processedData.cards) {
        ...
        doneRow.completed = now.to(moment_1.default(card['project_done_at']));
        rows.push(doneRow);
    }
    let table;
